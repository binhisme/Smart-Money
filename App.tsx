...
const App: React.FC = () => {
  ...
  /**
   * Kết quả tháng cập nhật:
   * Jan 2026: 7.95%
   * Feb 2026 (đến ngày 4): 1.12 + 0.84 + 1.31 + 0.95 = 4.22%
   * Tổng ROI: 12.17%
   */
  const monthlyResults = useMemo(() => [
    { id: 'm0', month: 'Tháng 02', year: 2026, result: 4.22, highest: false },
  ...
  const masters = useMemo(() => [
    { rank: 1, name: 'Apex Flow Execution (AFE)', pnl24h: 0.95, amount: 15432.7, private: true },
    { rank: 2, name: 'Precision Liquidity Strike (PLS)', pnl24h: 0.78, amount: 3890.0, private: false },
    { rank: 3, name: 'Quantum Pullback System (QPS)', pnl24h: 0.45, amount: 2570.0, private: false },
    { rank: 4, name: 'Adaptive Range Reversal (ARR)', pnl24h: -0.05, amount: 920.0, private: false },
    { rank: 5, name: 'Velocity Break Structure (VBS)', pnl24h: 0.18, amount: 1240.0, private: false },
  ], []);
  ...
  return (
    <div className="flex h-screen bg-black text-white font-sans overflow-hidden" id="app-root">
      <Sidebar 
        activeTab={activeTab} 
        ...
        <div className="flex-1 flex overflow-hidden relative">
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                  <div className="glass-card rounded-2xl p-5 lg:p-6 border border-white/5 relative group overflow-hidden">
                    ...
                    <div className="text-2xl lg:text-3xl font-black text-white mt-1">$ {calculatedAssets.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
                    <p className="text-[10px] text-green-500 mt-1 font-bold">+{financeData.profitRate}% Lợi nhuận ròng</p>
                  </div>
                  ...
                  <div className="glass-card rounded-2xl p-5 lg:p-6 border border-white/5">
                    <h4 className="text-gray-500 text-[10px] uppercase font-bold tracking-widest">ROI Tích lũy</h4>
                    <div className="text-xl lg:text-2xl font-black text-green-500 mt-1">+{stats.totalProfit}%</div>
                  </div>
                </div>
                      ...
                      <div key={idx} className={`relative rounded-xl p-4 lg:p-6 flex flex-col items-center text-center transition-all border border-white/5 ${idx === 0 ? 'bg-yellow-600/10 border-yellow-500/30' : 'bg-white/5'}`}>
                        ...
                        <div className="mb-4">
                          <p className={`text-[9px] font-black ${master.pnl24h >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                            PnL ({master.pnl24h >= 0 ? '+' : ''}{master.pnl24h}%)
                          </p>
                ...
                <div className="glass-card rounded-[1.5rem] p-6 lg:p-8">
                  <div className="flex items-center space-x-3 mb-8">
                    ...
                    <h3 className="font-black text-base lg:text-lg uppercase italic text-white tracking-tighter">Báo cáo PnL Trung bình</h3>
                  </div>
                    ...
                    <table className="w-full text-left">
                      <thead>
                        <tr className="text-[10px] uppercase text-gray-500 border-b border-white/5">
                          <th className="pb-4 font-black tracking-widest">Tháng giao dịch</th>
                          <th className="pb-4 font-black tracking-widest">Lợi nhuận gộp (%)</th>
                          <th className="pb-4 font-black tracking-widest">Lợi nhuận ròng ($)</th>
                          <th className="pb-4 font-black tracking-widest text-center">Trạng thái</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-white/5">
                        {monthlyResults.map((item) => (
                          <tr key={item.id} className="group hover:bg-white/[0.02] transition-colors">
                            <td className="py-4">
                              <span className="text-sm font-black text-gray-300 uppercase italic">{item.month} {item.year}</span>
                            </td>
                            <td className="py-4">
                              <span className="text-base lg:text-lg font-black text-green-500">+{item.result.toFixed(2)}%</span>
                            </td>
                            <td className="py-4 font-black text-xs text-gray-400">
                              $ {(financeData.deposited * item.result / 100).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            </td>
                            <td className="py-4 text-center">
                              <div className={`inline-flex items-center px-3 py-1 rounded text-[9px] font-black uppercase tracking-widest italic ${item.highest ? 'bg-yellow-500 text-black' : 'bg-green-500/10 text-green-500'}`}>
               ...
               <div className="h-full flex flex-col items-center justify-center space-y-4 text-gray-500 text-center">
                ...
                <p className="font-bold uppercase tracking-widest text-sm italic">Đang cập nhật...</p>
               </div>
            )}
          </div>
          ...
          >
               ...
               <div className={`p-4 lg:p-6 space-y-6 ${isHistoryExpanded ? 'max-w-5xl mx-auto w-full' : ''}`}>
                  <div className="grid grid-cols-2 gap-3">
                     <div className="bg-[#0a0a0a] border border-white/5 rounded-[1.5rem] p-4 lg:p-6 flex flex-col items-center justify-center">
                        <span className="text-[7px] lg:text-[9px] text-gray-500 font-black uppercase tracking-[0.2em] mb-2">PNL THÁNG</span>
                        <p className="text-lg lg:text-2xl font-black text-green-500 tracking-tighter">+{currentMonthData.result.toFixed(2)}%</p>
                     </div>
                     <div className="bg-[#0a0a0a] border border-white/5 rounded-[1.5rem] p-4 lg:p-6 flex flex-col items-center justify-center">
                        <span className="text-[7px] lg:text-[9px] text-gray-500 font-black uppercase tracking-[0.2em] mb-2">TỶ LỆ THẮNG</span>
                        <p className="text-lg lg:text-2xl font-black text-white tracking-tighter">{stats.winRate}%</p>
                     </div>
                  </div>
                    ...
                    {historyMode === 'grid' ? (
                          ...
                          {pnlHistoryFullMonth.map((item, idx) => {
                             ...
                             return (
                                <div key={idx} className={`aspect-square relative flex flex-col items-center justify-center rounded-xl border transition-all ${
                                   ...
                                   <span className="absolute top-1.5 left-1.5 text-[8px] font-black text-gray-600 tracking-tighter">{dayNum}</span>
                                   {hasData && (
                                      <p className={`text-[10px] font-black tracking-tighter ${item.isProfit ? 'text-green-500' : 'text-red-500'}`}>
                       ...
                       <div className="space-y-2">
                          {pnlHistoryFullMonth.filter(d => d.percentage !== 0).reverse().map((item, idx) => (
                             <div key={idx} className="bg-[#0a0a0a] border border-white/5 p-3 rounded-xl flex items-center justify-between group hover:bg-white/[0.03] transition-all">
                                <div className="flex items-center space-x-3">
                                   ...
                                   <div>
                                      <p className="text-[8px] text-gray-500 font-black italic">{item.date.split('-').reverse().join('/')}</p>
                                      <p className={`text-sm font-black tracking-tighter ${item.isProfit ? 'text-green-500' : 'text-red-500'}`}>
                                         {item.isProfit ? '+' : '-'}{Math.abs(item.percentage).toFixed(2)}%
                                      </p>
                                   </div>
                                </div>
                                <span className="text-[7px] font-black text-gray-600 uppercase tracking-widest italic">{item.strategy?.split(' ')[0]}</span>
                             </div>
                          ))}
                       </div>
                    )}
            ...
            <div className="bg-[#030303] border-t border-white/5 p-5 space-y-4 sticky bottom-0 z-40">
               <div className="flex items-center space-x-3">
                  <button className="flex-1 py-3 bg-red-600 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] text-white italic italic italic">SmartMoney-X AI</button>
               </div>
            </div>
          </div>
        </div>
